...
class LeaveApplicationController extends Controller
{
        ...
        if (($eligibility['insufficient_balance'] ?? false) === true) {
            ...
            if (LeaveType::isForcedLeaveType($leaveType, (int) $validated['leave_type_id'])) {
                return $this->buildInsufficientPrimaryLeaveBalanceResponse(
                    ...
                    (float) ($eligibility['required_balance_days'] ?? $deductibleDays)
                );
            }
    ...
    private function storeMonetization(Request $request, object $employee, object $account): JsonResponse
    {
        ...
        $monetizationBalanceRestriction = $this->validateMonetizationComponentBalances(
            (string) $employee->control_no,
        ...
        foreach ($admins as $deptAdmin) {
            Notification::send(
                ...
                trim(($employee->firstname ?? '').' '.($employee->surname ?? ''))." submitted a monetization request for {$monetizationLabel} (".self::formatDays($app->total_days).').',
                $app->id
            );
        }
    ...
    public function adminApprove(Request $request, int $id): JsonResponse
    {
        ...
        DB::transaction(function () use (
            ...
            if ($this->trimNullableString($logRemarks) === null && $isPendingApprovedUpdateRequest) {
                $logRemarks = $pendingApprovedActionType === LeaveApplicationUpdateRequest::ACTION_TYPE_CANCEL
                    ...
                    : (
                        $pendingApprovedActionType === LeaveApplicationUpdateRequest::ACTION_TYPE_RECALL
                            ? 'Approved leave recall request and forwarded to HR.'
        ...
        $notificationTitle = $isPendingApprovedUpdateRequest
            ? ($pendingApprovedActionType === LeaveApplicationUpdateRequest::ACTION_TYPE_CANCEL
                ...
                : (
                    $pendingApprovedActionType === LeaveApplicationUpdateRequest::ACTION_TYPE_RECALL
                        ? "{$titleLabel} Recall Request Pending HR Review"
        ...
        $notificationMessage = $isPendingApprovedUpdateRequest
            ? ($pendingApprovedActionType === LeaveApplicationUpdateRequest::ACTION_TYPE_CANCEL
                ...
                : (
                    $pendingApprovedActionType === LeaveApplicationUpdateRequest::ACTION_TYPE_RECALL
                        ? "{$employeeName} submitted a recall request for an approved {$app->leaveType->name} {$actionLabel}. The request has been approved by department admin and awaits your review."
        ...
        return response()->json([
            'message' => $isPendingApprovedUpdateRequest
                ? ($pendingApprovedActionType === LeaveApplicationUpdateRequest::ACTION_TYPE_CANCEL
                    ...
                    : (
                        $pendingApprovedActionType === LeaveApplicationUpdateRequest::ACTION_TYPE_RECALL
                            ? 'Leave recall request approved by admin. Forwarded to HR for final approval.'
    ...
    public function adminRequestRecall(Request $request, int $id): JsonResponse
    {
        $admin = $request->user();
        if (! $admin instanceof DepartmentAdmin) {
            return response()->json(['message' => 'Only department admins can request recalls.'], 403);
        }
        ...
        $app = LeaveApplication::with(['leaveType', 'applicantAdmin'])->find($id);
        if (! $app) {
            return response()->json(['message' => 'Leave application not found.'], 404);
        }
        ...
        if ($app->status !== LeaveApplication::STATUS_APPROVED) {
            return response()->json([
                'message' => "Cannot request recall: application status is '{$app->status}', expected 'APPROVED'.",
            ], 422);
        }
        ...
        $remarksLine = $reason !== ''
            ? "Recall request submitted by department admin. Reason: {$reason}"
            : 'Recall request submitted by department admin.';

        $requestedUpdatePayload = $this->buildRequestedLeaveRecallPayload(
            $app,
            $reason,
            $selectedRecallDateKeys
        );

        $requestedByControlNo = trim((string) (
        ...
        DB::transaction(function () use (
            ...
            $requestPayload = [
                ...
                'previous_status' => LeaveApplication::STATUS_APPROVED,
                'requested_by_control_no' => $requestedByControlNo !== '' ? $requestedByControlNo : (string) ($app->employee_control_no ?? ''),
                'requested_at' => now(),
            ];
        ...
        $employeeName = $this->resolveEmployeeDisplayName($app);
        $message = "{$employeeName} requested a recall for an approved {$leaveType->name} leave application.";
        if ($reason !== '') {
            $message .= " Reason: {$reason}";
        }
        ...
        foreach ($hrAccounts as $hrAccount) {
            Notification::send(
                $hrAccount,
                Notification::TYPE_LEAVE_EDIT_REQUEST,
                'Recall Request Pending HR Review',
                $message,
                $app->id
            );
        }
        ...
        return response()->json([
            'message' => 'Recall request submitted and forwarded to HR for approval.',
    ...
    public function hrUndoReceive(Request $request, int $id): JsonResponse
    {
        $hr = $request->user();
        if (! $hr instanceof HRAccount) {
            return response()->json(['message' => 'Only HR accounts can undo received applications.'], 403);
        }
        ...
        $cycleRequestedAt = $this->resolveCurrentHrDocumentCycleRequestedAt($app);
        $releasedLog = $this->resolveLatestHrActionLogForCycle(
            $app,
            LeaveApplicationLog::ACTION_HR_RELEASED,
            $cycleRequestedAt
        );
        if ($releasedLog instanceof LeaveApplicationLog) {
            return response()->json([
                'message' => 'Cannot undo receipt after release. Undo the release first.',
            ], 422);
        }

        $reviewedLog = $this->resolveLatestHrActionLogForCycle(
            $app,
            LeaveApplicationLog::ACTION_CMO_CBMO_REVIEWED,
            $cycleRequestedAt
        );
        if ($reviewedLog instanceof LeaveApplicationLog) {
            return response()->json([
                'message' => 'Cannot undo receipt after CMO/CVMO review has been completed.',
            ], 422);
        }
        ...
        if ($receivedLog instanceof LeaveApplicationLog) {
            $receivedLog->delete();
            $app->load('logs');
        }
        ...
        return response()->json([
            'message' => $receivedLog instanceof LeaveApplicationLog
                ? 'Hard-copy receipt was undone.'
                : 'Hard-copy receipt is not currently confirmed for this application.',
            'application' => $this->formatErmsApplication($app, $actorDirectory),
        ]);
    }
    ...
    public function hrUndoRelease(Request $request, int $id): JsonResponse
    {
        $hr = $request->user();
        if (! $hr instanceof HRAccount) {
            return response()->json(['message' => 'Only HR accounts can undo released applications.'], 403);
        }
        ...
        $releasedLog = $this->resolveLatestHrActionLogForCycle(
            $app,
            LeaveApplicationLog::ACTION_HR_RELEASED,
            $this->resolveCurrentHrDocumentCycleRequestedAt($app)
        );
        ...
        return response()->json([
            'message' => $releasedLog instanceof LeaveApplicationLog
                ? 'Hard-copy release was undone.'
                : 'Hard-copy release is not currently confirmed for this application.',
            'application' => $this->formatErmsApplication($app, $actorDirectory),
        ]);
    }
                    ...
                    $pendingActionType === LeaveApplicationUpdateRequest::ACTION_TYPE_CANCEL
                        ...
                        : (
                            $pendingActionType === LeaveApplicationUpdateRequest::ACTION_TYPE_RECALL
                                ? 'Received leave recall request form.'
        ...
        return response()->json([
            'message' => $receivedLog
                ? ($pendingActionType === LeaveApplicationUpdateRequest::ACTION_TYPE_CANCEL
                    ...
                    : (
                        $pendingActionType === LeaveApplicationUpdateRequest::ACTION_TYPE_RECALL
                            ? 'Recall request hard-copy receipt was already confirmed.'
                ...
                : ($pendingActionType === LeaveApplicationUpdateRequest::ACTION_TYPE_CANCEL
                    ...
                    : (
                        $pendingActionType === LeaveApplicationUpdateRequest::ACTION_TYPE_RECALL
                            ? 'Recall request hard-copy receipt confirmed.'
    ...
    public function hrReleaseUpdate(Request $request, int $id): JsonResponse
    {
        ...
        $allowedStatuses = $latestApprovedActionType === LeaveApplicationUpdateRequest::ACTION_TYPE_CANCEL
            ...
            : (
                $latestApprovedActionType === LeaveApplicationUpdateRequest::ACTION_TYPE_RECALL
                    ? [LeaveApplication::STATUS_APPROVED, LeaveApplication::STATUS_RECALLED]
                    : [LeaveApplication::STATUS_APPROVED]
            );
        if (! in_array($app->status, $allowedStatuses, true)) {
            return response()->json([
                'message' => $latestApprovedActionType === LeaveApplicationUpdateRequest::ACTION_TYPE_CANCEL
                    ...
                    : (
                        $latestApprovedActionType === LeaveApplicationUpdateRequest::ACTION_TYPE_RECALL
                            ? "Cannot mark recall request copy as released: application status is '{$app->status}'."
            ...
            if (! $receivedLog) {
                return response()->json([
                    'message' => $latestApprovedActionType === LeaveApplicationUpdateRequest::ACTION_TYPE_CANCEL
                        ...
                        : (
                            $latestApprovedActionType === LeaveApplicationUpdateRequest::ACTION_TYPE_RECALL
                                ? 'Cannot mark recall request copy as released before confirming recall request hard-copy receipt.'
        ...
        return response()->json([
            'message' => $releasedLog
                ? ($latestApprovedActionType === LeaveApplicationUpdateRequest::ACTION_TYPE_CANCEL
                    ...
                    : (
                        $latestApprovedActionType === LeaveApplicationUpdateRequest::ACTION_TYPE_RECALL
                            ? 'Recall request hard-copy release was already confirmed.'
                ...
                : ($latestApprovedActionType === LeaveApplicationUpdateRequest::ACTION_TYPE_CANCEL
                    ...
                    : (
                        $latestApprovedActionType === LeaveApplicationUpdateRequest::ACTION_TYPE_RECALL
                            ? 'Recall request hard-copy release confirmed.'
    ...
    public function hrApprove(Request $request, int $id): JsonResponse
    {
        ...
        $attachmentReference = $policyResolution['attachment_reference'] ?? null;
        if ((bool) $app->is_monetization) {
            $attachmentRequired = (bool) ($app->attachment_required ?? false);
            $attachmentSubmitted = (bool) ($app->attachment_submitted ?? false);
            $attachmentReference = $this->trimNullableString($app->attachment_reference ?? null);
        }
        ...
        if ((bool) $app->is_monetization) {
            ...
            $monetizationBalanceRestriction = $this->validateStoredMonetizationComponentBalances($app, $leaveType);
            if ($monetizationBalanceRestriction instanceof JsonResponse) {
                return $monetizationBalanceRestriction;
            }
        }

        if ($needsDeduction && ! (bool) $app->is_monetization) {
            if ($app->employee_control_no) {
                // Lock the selected leave type balance before approval to catch stale balances.
                ...
                if (
                    ...
                    && $requiredPrimaryLeaveDays > 0.0
                    && (! $balance || (float) $balance->balance < $requiredPrimaryLeaveDays)
                ) {
        ...
        try {
            ...
            if ((bool) $app->is_monetization) {
                ...
                return response()->json([
                    'message' => 'Unable to approve monetization because leave balances changed. Please review the current balances and try again.',
                ], 422);
            }
    ...
    public function hrRecall(Request $request, int $id): JsonResponse
    {
        ...
        $reason = trim((string) ($validated['recall_reason'] ?? $validated['remarks'] ?? ''));
        $recallSelectedDates = is_array($validated['recall_selected_dates'] ?? null)
            ? $validated['recall_selected_dates']
            : [];

        return $this->executeHrRecall($app, $hr, $reason, $recallSelectedDates);
    }

    private function executeHrRecall(
        LeaveApplication $app,
        HRAccount $hr,
        string $reason,
        array $requestedRecallDates,
        ?LeaveApplicationUpdateRequest $pendingUpdateRequest = null,
        mixed $pendingReviewRemarks = null
    ): JsonResponse {
        if ((bool) $app->is_monetization) {
            return response()->json([
                'message' => 'Monetization requests cannot be recalled.',
        ...
        $allowedStatuses = $pendingUpdateRequest instanceof LeaveApplicationUpdateRequest
            ? [LeaveApplication::STATUS_PENDING_HR, LeaveApplication::STATUS_APPROVED]
            : [LeaveApplication::STATUS_APPROVED];
        if (! in_array($app->status, $allowedStatuses, true)) {
            return response()->json([
                'message' => $pendingUpdateRequest instanceof LeaveApplicationUpdateRequest
                    ? "Cannot approve recall request: application status is '{$app->status}'."
                    : "Cannot recall: application status is '{$app->status}', expected 'APPROVED'.",
            ], 422);
        }
            ...
            DB::transaction(function () use (
                ...
                if ($pendingUpdateRequest instanceof LeaveApplicationUpdateRequest) {
                    $pendingUpdateRequest->update([
                        ...
                        'reviewed_by_hr_id' => $hr->id,
                        'reviewed_at' => now(),
                        'review_remarks' => $this->trimNullableString($pendingReviewRemarks),
                    ]);
                }
        ...
        if ($app->applicant_admin_id && $app->applicantAdmin) {
            Notification::send(
                $app->applicantAdmin,
                Notification::TYPE_LEAVE_CANCELLED,
                $pendingUpdateRequest instanceof LeaveApplicationUpdateRequest
                    ? 'Recall Request Approved by HR'
                    : 'Leave Recalled by HR',
                $pendingUpdateRequest instanceof LeaveApplicationUpdateRequest
                    ? "Your {$app->leaveType->name} recall request was approved by HR.".($reason !== '' ? " Reason: {$reason}" : '')
        ...
        return response()->json([
            'message' => $pendingUpdateRequest instanceof LeaveApplicationUpdateRequest
                ? ($isFullyRecalled
                    ? 'Recall request approved by HR. Application recalled.'
                    : 'Recall request approved by HR. Selected leave dates were recalled.')
                : ($isFullyRecalled
    ...
    private function adminStoreMonetization(Request $request, DepartmentAdmin $admin): JsonResponse
    {
        ...
        foreach ($hrAccounts as $hrAccount) {
            Notification::send(
                ...
                "{$employeeName} filed a {$monetizationLabel} monetization request (".self::formatDays($app->total_days).') through department admin and it awaits your review.',
                $app->id
            );
        }
    ...
    private function restoreApplicationBalance(
        ...
        $restoreAmount = round(max($daysToRestore, 0.0), 3);
    ...
    private function loadEmployeeLeaveDeductionHistoryByType(string $controlNo, ?int $leaveTypeId = null): array
    {
        ...
        foreach ($applications as $application) {
            ...
            $creditsDeducted = round((float) ($application->deductible_days ?? $application->total_days ?? 0), 3);
    ...
    private function buildRequestedLeaveRecallPayload(
        LeaveApplication $app,
        ...
        $leaveTypeId = $this->resolveCanonicalLeaveTypeId((int) ($app->leave_type_id ?? 0))
            ?? (int) ($app->leave_type_id ?? 0);
        $normalizedRecallDateKeys = $this->normalizeRecallDateKeys($selectedRecallDateKeys);
    ...
    private function hasRequestedLeaveUpdateChanges(LeaveApplication $app, array $requestedPayload): bool
    {
        ...
        if (round((float) ($currentPayload['deductible_days'] ?? 0), 3) !== round((float) ($normalizedRequestedPayload['deductible_days'] ?? 0), 3)) {
            return true;
        }
    ...
    private function resolveUpdateRequestActionTypeFromPayload(mixed $payload): string
    {
        ...
        $recallFlagValues = [
            $payload['recall_leave'] ?? null,
            $payload['request_recall'] ?? null,
            $payload['is_recall_request'] ?? null,
        ];
        foreach ($recallFlagValues as $value) {
            if ($value === true || $value === 1 || $value === '1' || $value === 'true') {
                return LeaveApplicationUpdateRequest::ACTION_TYPE_RECALL;
            }
        }
    ...
    private function withRequestedLeaveActionType(array $payload, string $actionType): array
    {
        ...
        $requestKind = $normalizedActionType === LeaveApplicationUpdateRequest::ACTION_TYPE_CANCEL
            ? 'cancel'
            : ($normalizedActionType === LeaveApplicationUpdateRequest::ACTION_TYPE_RECALL ? 'recall' : 'update');

        return array_merge($payload, [
            'action_type' => $normalizedActionType,
            'request_kind' => $requestKind,
            'cancel_leave' => $normalizedActionType === LeaveApplicationUpdateRequest::ACTION_TYPE_CANCEL,
            'recall_leave' => $normalizedActionType === LeaveApplicationUpdateRequest::ACTION_TYPE_RECALL,
        ]);
    }
    ...
    private function hrApprovePendingUpdateRequest(Request $request, LeaveApplication $app, HRAccount $hr): JsonResponse
    {
        ...
        if ($pendingActionType === LeaveApplicationUpdateRequest::ACTION_TYPE_RECALL) {
            return $this->hrApprovePendingRecallRequest(
                $request,
                $app,
                $hr,
                $pendingUpdateRequest,
    ...
    private function hrApprovePendingRecallRequest(
        Request $request,
        LeaveApplication $app,
        HRAccount $hr,
        ...
        $payload = is_array($pendingUpdateMeta['payload'] ?? null)
            ? $pendingUpdateMeta['payload']
            : [];
        $recallReason = trim((string) (
        ...
        $recallSelectedDates = is_array($payload['recall_selected_dates'] ?? null)
            ? $payload['recall_selected_dates']
            : (is_array($payload['selected_dates'] ?? null) ? $payload['selected_dates'] : []);

        return $this->executeHrRecall(
            $app,
            $hr,
            $recallReason,
            $recallSelectedDates,
            $pendingUpdateRequest instanceof LeaveApplicationUpdateRequest ? $pendingUpdateRequest : null,
            $request->input('remarks')
        );
    }
    ...
    private function hrRejectPendingUpdateRequest(Request $request, LeaveApplication $app, HRAccount $hr): JsonResponse
    {
        ...
        return response()->json([
            'message' => $pendingActionType === LeaveApplicationUpdateRequest::ACTION_TYPE_CANCEL
                ...
                : (
                    $pendingActionType === LeaveApplicationUpdateRequest::ACTION_TYPE_RECALL
                        ? 'Leave recall request rejected by HR. Original approved application remains unchanged.'
    ...
    private function adminRejectPendingUpdateRequest(
        ...
        return response()->json([
            'message' => $pendingActionType === LeaveApplicationUpdateRequest::ACTION_TYPE_CANCEL
                ...
                : (
                    $pendingActionType === LeaveApplicationUpdateRequest::ACTION_TYPE_RECALL
                        ? 'Leave recall request rejected by admin. Original approved application remains unchanged.'
    ...
    private function resolveCurrentHrDocumentCycleRequestedAt(LeaveApplication $app): ?\DateTimeInterface
    {
        if ($this->isPendingApprovedUpdateRequest($app)) {
            return $this->resolvePendingApprovedUpdateCycleRequestedAt($app);
        }

        return $this->resolveLatestApprovedUpdateCycleRequestedAt($app);
    }
    ...
    private function normalizePendingUpdatePayload(mixed $payload): ?array
    {
        ...
        if ($actionType === LeaveApplicationUpdateRequest::ACTION_TYPE_RECALL) {
            ...
            $recallReason = $this->trimNullableString(
                $payload['recall_reason']
                ?? $payload['reason']
            ...
            $selectedDates = is_array($payload['selected_dates'] ?? null)
                ? LeaveApplication::resolveDateSet(null, null, $payload['selected_dates'], null)
                : null;
            $recallSelectedDates = $this->normalizeRecallDateKeys(
                is_array($payload['recall_selected_dates'] ?? null)
                    ? $payload['recall_selected_dates']
                    : (is_array($selectedDates) ? $selectedDates : [])
            );
            $totalDays = round((float) (
                $payload['total_days']
                ?? ($recallSelectedDates !== [] ? count($recallSelectedDates) : 0)
            ), 2);

            return [
                'action_type' => LeaveApplicationUpdateRequest::ACTION_TYPE_RECALL,
                'request_kind' => 'recall',
                'cancel_leave' => false,
                'recall_leave' => true,
                'leave_type_id' => $leaveTypeId > 0 ? $leaveTypeId : null,
                'leave_type_name' => $leaveTypeName,
                'start_date' => $this->trimNullableString($payload['start_date'] ?? null),
                'end_date' => $this->trimNullableString($payload['end_date'] ?? null),
                'selected_dates' => $selectedDates,
                'recall_selected_dates' => $recallSelectedDates,
                'total_days' => $totalDays,
                'deductible_days' => round((float) ($payload['deductible_days'] ?? $totalDays), 3),
                'reason' => $recallReason,
                'recall_reason' => $recallReason,
                'details_of_leave' => $this->trimNullableString(
                    $payload['details_of_leave'] ?? $payload['detailsOfLeave'] ?? null
                ),
                'commutation' => $this->trimNullableString($payload['commutation'] ?? null) ?? 'Not Requested',
                'pay_mode' => $this->normalizePayMode(
                    $payload['pay_mode'] ?? null,
                    (bool) ($payload['is_monetization'] ?? false)
                ),
                'is_monetization' => (bool) ($payload['is_monetization'] ?? false),
            ];
        }
    ...
    private function refundApplicationTrackedDeductions(
        ...
        if ((bool) $app->is_monetization) {
            foreach ($this->resolveStoredMonetizationLeaveCreditComponents($app, $sourceLeaveType) as $component) {
                $this->incrementEmployeeLeaveBalance(
                    $employeeControlNo,
                    (int) $component['leave_type_id'],
    ...
    private function deductApplicationTrackedBalances(
        ...
        $normalizedDaysToDeduct = round(max($daysToDeduct, 0.0), 3);
        ...
        $isForcedLeaveApplication = ! (bool) $app->is_monetization
            && $forcedLeaveTypeId !== null
            && $primaryLeaveTypeId === $forcedLeaveTypeId;

        if ((bool) $app->is_monetization) {
            foreach ($this->resolveStoredMonetizationLeaveCreditComponents($app) as $component) {
                $balance = $this->lockEmployeeLeaveBalance($employeeControlNo, (int) $component['leave_type_id']);
                $componentDays = round(max((float) $component['days'], 0.0), 3);
                if (! $balance || (float) $balance->balance + 1e-9 < $componentDays) {
                    throw new \RuntimeException($balanceConflictError);
                }

                if ($componentDays > 0.0) {
                    $balance->decrement('balance', $componentDays);
                }
            }
        ...
        $linkedForcedLeaveDeductedDays = 0.0;
        if (($shouldDeductForcedLeave || $isForcedLeaveApplication) && $forcedLeaveTypeId !== null) {
            $forcedBalance = $isForcedLeaveApplication && $primaryLeaveTypeId === $forcedLeaveTypeId
                ? $primaryBalance
    ...
    private function incrementEmployeeLeaveBalance(string $employeeControlNo, int $leaveTypeId, float $daysToAdd): void
    {
        ...
        $normalizedDaysToAdd = round(max($daysToAdd, 0.0), 3);
    ...
    private function resolveStoredLinkedForcedLeaveDeduction(
        ...
        if ($app->linked_forced_leave_deducted_days !== null) {
            return round(max((float) $app->linked_forced_leave_deducted_days, 0.0), 3);
        }
        ...
        return round(max($fallbackDeductibleDays, 0.0), 3);
    }
    ...
    private function resolveStoredLinkedVacationLeaveDeduction(
        ...
        if ($app->linked_vacation_leave_deducted_days !== null) {
            return round(max((float) $app->linked_vacation_leave_deducted_days, 0.0), 3);
        }
        ...
        return $this->resolveLeaveTypeVacationLeaveDeductionDays(
            ...
            (int) $app->leave_type_id,
            (bool) $app->is_monetization,
            round((float) ($app->total_days ?? $fallbackDeductibleDays), 3),
            $fallbackDeductibleDays,
            $forcedLeaveTypeId,
            $vacationLeaveTypeId
        );
    ...
    {
        return LeaveType::resolveForcedLeaveTypeId();
    }
    ...
    private function resolvePrimaryLeaveTrackedDeductionDays(
        ...
        $normalizedDeductibleDays = round(max($deductibleDays, 0.0), 3);
        $canonicalLeaveTypeId = $this->resolveCanonicalLeaveTypeId($leaveTypeId) ?? $leaveTypeId;
        if (! $isMonetization && $forcedLeaveTypeId !== null && $canonicalLeaveTypeId === $forcedLeaveTypeId) {
            return 0.0;
        }
    ...
    private function normalizeMonetizationLeaveCreditComponents(array $validated, ?LeaveType $fallbackLeaveType = null): array|JsonResponse
    {
        ...
        if (count($componentsByType) > 2) {
            return response()->json([
                'message' => 'Monetization may only include Vacation Leave and Sick Leave.',
                'errors' => [
                    'monetization_leave_credits' => ['Monetization may only include Vacation Leave and Sick Leave.'],
                ],
            ], 422);
        }

        $components = collect($componentsByType)
            ->sortBy(fn (array $component): int => $this->isVacationLeaveType(null, (int) $component['leave_type_id']) ? 0 : 1)
            ->values()
            ->all();
     ...
     * @return array<int, array{leave_type_id:int, leave_type_name:string, days:float}>
     */
    private function resolveStoredMonetizationLeaveCreditComponents(
        LeaveApplication $app,
        ?LeaveType $fallbackLeaveType = null
    ): array {
        ...
        if (is_array($rawComponents)) {
            foreach ($rawComponents as $rawComponent) {
                ...
                if ($leaveTypeName === null) {
                    $leaveTypeName = LeaveType::query()->whereKey($canonicalLeaveTypeId)->value('name');
                }

                $components[] = [
                    'leave_type_id' => $canonicalLeaveTypeId,
                    'leave_type_name' => (string) ($leaveTypeName ?: 'Leave'),
                    'days' => $days,
                ];
            }
        }
        ...
        $fallbackLeaveType ??= $app->leaveType instanceof LeaveType
            ? $app->leaveType
            : LeaveType::query()->find((int) $app->leave_type_id);
        $fallbackLeaveTypeId = (int) ($this->resolveCanonicalLeaveTypeId((int) ($fallbackLeaveType?->id ?? $app->leave_type_id)) ?? (int) $app->leave_type_id);

        if ($fallbackLeaveTypeId <= 0) {
            return [];
        }

        return [[
            'leave_type_id' => $fallbackLeaveTypeId,
            'leave_type_name' => (string) ($fallbackLeaveType?->name ?? 'Leave'),
    ...
    {
        $requestedLeaveTypeId = $this->resolveCanonicalLeaveTypeId($requestedLeaveTypeId) ?? $requestedLeaveTypeId;
     ...
     * @param  array<int, array{leave_type_id:int, leave_type_name:string, days:float}>  $components
     */
    private function validateMonetizationComponentBalances(string $employeeControlNo, array $components): ?JsonResponse
    {
        foreach ($components as $component) {
            $balance = $this->findPreferredEmployeeLeaveBalanceRecord(
                $employeeControlNo,
    ...
    private function validateStoredMonetizationComponentBalances(LeaveApplication $app, ?LeaveType $fallbackLeaveType = null): ?JsonResponse
    {
        $employeeControlNo = $this->resolveApplicationBalanceOwnerControlNo($app);
        if ($employeeControlNo === null) {
            return response()->json([
                'message' => 'Unable to identify the employee leave balances for this monetization request.',
            ], 422);
        }

        return $this->validateMonetizationComponentBalances(
            $employeeControlNo,
            $this->resolveStoredMonetizationLeaveCreditComponents($app, $fallbackLeaveType)
        );
    }
    ...
    private function validateMonetizationLeaveTypeEligibility(?LeaveType $leaveType, string $errorKey = 'leave_type_id'): ?JsonResponse
    {
        ...
        if (! $isEligibleMonetizationType) {
            return response()->json([
                ...
                'errors' => [
                    $errorKey => ['Monetization is only allowed for Vacation Leave or Sick Leave.'],
                ],
            ], 422);
        }
    ...
    {
        $normalizedRequestedDays = round(max($requestedDays, 0.0), 3);
        if ($normalizedRequestedDays + 1e-9 < LeaveApplication::MONETIZATION_MINIMUM_REQUEST_DAYS) {
            ...
            return response()->json([
                ...
                'errors' => [
                    $errorKey => ["A minimum of {$minimumDays} is required for monetization."],
                ],
            ], 422);
        }
    ...
    private function validateMonetizationBalanceRules(
        ...
        $normalizedCurrentBalance = round(max($currentBalance, 0.0), 3);
        $normalizedRequestedDays = round(max($requestedDays, 0.0), 3);
    ...
    private function applyRegularLeavePolicy(
        ...
        return [
            ...
            'deductible_days' => round(max((float) $deductibleDays, 0.0), 3),
    ...
    private function buildSickLeavePayStatusOverrides(
        ...
        $remainingWithPay = round(max($withPayCap, 0.0), 3);
        $statusOverrides = [];
        ...
        foreach ($resolvedDates as $dateKey) {
            ...
            $remainingWithPay = round(max($remainingWithPay - $weight, 0.0), 3);
        }
    ...
    private function resolveApplicationDeductibleDays(LeaveApplication $app): float
    {
        ...
        if ($app->deductible_days !== null) {
            $stored = round((float) $app->deductible_days, 3);
            if ($stored < 0) {
                return 0.0;
            }
        ...
        if ($storedCtoHours > 0 && $this->isCtoLeaveType($app->leaveType, (int) $app->leave_type_id)) {
            return round($storedCtoHours / self::CTO_STANDARD_DAY_HOURS, 3);
        }
    ...
    private function resolveRecallRestorableDetails(
        ...
        $coverageWeights = $this->resolveDateCoverageWeights(
            $selectedDates,
            $normalizedCoverage,
            round((float) ($app->total_days ?? 0), 3),
            (string) ($app->employee_control_no ?? '')
        );
        ...
        $restorableDays = round(max($restorableDays, 0.0), 3);
    ...
    private function validateRegularLeaveEligibility(
        ...
        if ($requiresVacationLeaveTopUp) {
            $requiredVacationLeaveDays = round(max($requiredBalanceDays - max($availableBalance, 0.0), 0.0), 3);
            $requiredBalanceDays = round(max($requiredBalanceDays - $requiredVacationLeaveDays, 0.0), 3);
    ...
    {
        $availableBalance = round(max((float) ($eligibility['available_balance'] ?? 0.0), 0.0), 3);
        $requiredBalance = round(max((float) ($eligibility['required_balance_days'] ?? 0.0), 0.0), 3);
    ...
    private function resolveEmployeeLeaveBalanceSnapshot(string $employeeControlNo, int $leaveTypeId): array
    {
        ...
        if ($isCtoSnapshot) {
            ...
            $currentBalance = round($currentBalanceHours / self::CTO_STANDARD_DAY_HOURS, 3);
            $pendingReservedDays = round($pendingReservedHours / self::CTO_STANDARD_DAY_HOURS, 3);
            $availableBalance = round($availableBalanceHours / self::CTO_STANDARD_DAY_HOURS, 3);
        ...
        $availableBalance = max(round($currentBalance - $pendingReservedDays, 3), 0.0);
    ...
    private function resolvePendingReservedDaysForTrackedLeaveType(string $employeeControlNo, int $leaveTypeId): float
    {
        ...
        foreach ($pendingApplications as $pendingApplication) {
            ...
            $applicationTotalDays = round((float) ($pendingApplication->total_days ?? $applicationDeductibleDays), 3);
        ...
        return round(max($pendingReservedDays, 0.0), 3);
    }
    ...
    private function formatApplication(LeaveApplication $app): array
    {
        ...
        $data = [
            ...
            'certificationLeaveCredits' => $this->resolveCertificationLeaveCredits($app, $leaveBalanceSnapshot),
    ...
    /**
     * Reprice future pending leave applications using the current
     * work-schedule deductions.
     *
     * Approved applications are intentionally excluded so historical
     * approved records remain unchanged when schedules are updated.
     *
     * @return array{
     *   checked:int,
     *   repriced:int,
    ...
    public function repriceFutureApprovedApplicationsForScheduleChange(?string $employeeControlNo = null): array
    {
        ...
        foreach ($applications as $application) {
            ...
            $targetPayMode = $policyResolution['pay_mode'];
            $targetSelectedDatePayStatus = $policyResolution['selected_date_pay_status'];
            $targetDeductibleDays = round(max((float) ($policyResolution['deductible_days'] ?? 0.0), 0.0), 3);
            ...
            $targetForcedAvailableBeforeApplication = $this->resolveTrackedLeaveBalanceBeforeApplicationDeduction(
                $application,
                $forcedLeaveTypeId,
                $sourceLinkedForcedDeduction
            );
            ...
            $isTargetForcedLeaveApplication = $forcedLeaveTypeId !== null
                && $canonicalLeaveTypeId === $forcedLeaveTypeId;
            $targetLinkedForcedDeductionBase = $isTargetForcedLeaveApplication
                ? $targetDeductibleDays
                : $targetPrimaryDeduction;
            $targetLinkedForcedDeduction = (
                $targetDeductsBalance
                && (
                    $isTargetForcedLeaveApplication
                    || $this->shouldLeaveTypeDeductForcedLeave(
                        $leaveType,
                        $canonicalLeaveTypeId,
                        false,
                ...
                ? round(min(max($targetLinkedForcedDeductionBase, 0.0), $targetForcedAvailableBeforeApplication), 3)
                : 0.0;
            ...
            if (abs($deltaLinkedForcedDeduction) < 0.001) {
            ...
            $hasApplicationFieldChanges = $sourcePayMode !== $targetPayMode
                || round($sourceDeductibleDays, 3) !== $targetDeductibleDays
    ...
    private function applyScheduleRepricingBalanceDelta(
        ...
        $normalizedDeltaDays = round($deltaDays, 3);
        if ($leaveTypeId <= 0 || abs($normalizedDeltaDays) < 0.001) {
            return;
        }
    ...
    private function getFreshApplicationLeaveBalanceSnapshot(LeaveApplication $app, bool $syncCtoBalance = true): array
    {
        $lookupControlNo = $this->resolveApplicationBalanceLookupControlNo($app);
        if ($lookupControlNo === null) {
            return [];
        ...
        return collect($this->mapLeaveBalancesByCanonicalTypeId(LeaveBalance::query()
            ->with('leaveType')
            ->whereIn('employee_control_no', $controlNoCandidates)
            ->get()))
            ->sortBy(fn (LeaveBalance $balance) => strtolower(trim((string) ($balance->leaveType?->name ?? ''))))
    ...
    private function lockCertificationLeaveCreditsSnapshot(LeaveApplication $app): void
    {
        ...
        $app->loadMissing('leaveType');
        $leaveBalanceSnapshot = $this->getFreshApplicationLeaveBalanceSnapshot($app, false);
        $snapshot = $this->buildCertificationLeaveCredits($app, $leaveBalanceSnapshot);
        $snapshot['as_of_date'] = $app->hr_approved_at?->format('F j, Y') ?? now()->format('F j, Y');
    ...
    private function findLeaveTypeBalanceByNameInSnapshot(array $snapshot, string $leaveTypeName): float
    {
        foreach ($snapshot as $entry) {
            if (strcasecmp((string) ($entry['leave_type_name'] ?? ''), $leaveTypeName) === 0) {
                return $this->roundLeaveCreditValue((float) ($entry['balance'] ?? 0.0));
            }
        }
    ...
    private function reconstructApprovedCertificationLeaveCredits(
        ...
        $snapshot = $leaveBalanceSnapshot !== []
            ? $leaveBalanceSnapshot
            : $this->getApplicationLeaveBalanceSnapshot($app);
    ...
    private function resolveCertificationLeaveDeductionsByType(LeaveApplication $app): array
    {
        ...
        if ((bool) $app->is_monetization) {
            foreach ($this->resolveStoredMonetizationLeaveCreditComponents($app) as $component) {
                if ($this->isVacationLeaveType(null, (int) $component['leave_type_id'])) {
                    $vacation += (float) $component['days'];
                } elseif ($this->isSickLeaveType(null, (int) $component['leave_type_id'])) {
        ...
        } elseif ($isForcedLeave) {
            $vacation = $linkedVacationDeduction > 0.0
                ? $linkedVacationDeduction
                : $deductibleDays;
    ...
    private function buildCertificationLeaveCredits(LeaveApplication $app, array $leaveBalanceSnapshot = []): array
    {
        ...
        if ((bool) $app->is_monetization) {
            foreach ($this->resolveStoredMonetizationLeaveCreditComponents($app) as $component) {
                if ($this->isVacationLeaveType(null, (int) $component['leave_type_id'])) {
                    $vacationLessThisApplication += (float) $component['days'];
                } elseif ($this->isSickLeaveType(null, (int) $component['leave_type_id'])) {
                    $sickLessThisApplication += (float) $component['days'];
                }
            }
        } elseif (strcasecmp($leaveTypeName, 'Vacation Leave') === 0) {
            $primaryDeductionDays = $normalizedPayMode === LeaveApplication::PAY_MODE_WITHOUT_PAY
            ...
            $vacationLessThisApplication = $linkedVacationDeduction > 0.0
                ? $linkedVacationDeduction
                : $deductibleDays;
        } elseif ($linkedVacationDeduction > 0.0) {
            $vacationLessThisApplication = $linkedVacationDeduction;
        ...
        $vacationLessThisApplication = $this->roundLeaveCreditValue($vacationLessThisApplication);
        $sickLessThisApplication = $this->roundLeaveCreditValue($sickLessThisApplication);

        $vacationTotalEarned = $this->roundLeaveCreditValue($deductionAlreadyApplied
            ? ($vacationCurrentBalance + $vacationLessThisApplication)
        ...
        $sickTotalEarned = $this->roundLeaveCreditValue($deductionAlreadyApplied
            ...
            : $sickCurrentBalance);
        $vacationBalanceAfterApplication = $this->roundLeaveCreditValue($deductionAlreadyApplied
            ? $vacationCurrentBalance
            : ($vacationCurrentBalance - $vacationLessThisApplication));
